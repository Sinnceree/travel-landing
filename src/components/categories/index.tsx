...
const Categories: React.FC = () => {
  return (
    <div className='flex flex-col my-10'>
      <h1 className='text-white font-bold text-3xl'>Top Categories</h1>

      <div className='flex justify-between w-full my-7 flex-wrap'>
        {CATEGORY_ITEMS.map((c, i) => (
          <div key={i} className='flex flex-col basis-1/3 items-center justify-center md:basis-1/12 lg:flex'>
            <img src={c.icon} className='h-9'/>
            <h1 className='text-white/90 my-3 text-base'>{c.title}</h1>
          </div>
        ))}
      </div>
    </div>
