...
const DestinationItem: React.FC<DestinationItemProps> = ({ image, title }) => {
  ...
  return (
    <div className='flex min-w-[20rem] h-48 bg-cover bg-no-repeat bg-center rounded-xl relative overflow-hidden' onMouseEnter={() => setHovering(true)} onMouseLeave={() => setHovering(false)}>
      <h1 className='text-white font-semibold text-lg flex items-end pb-3 pl-4 z-10'>{title}</h1>
